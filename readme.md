...
一个轻量、常驻系统托盘的番茄钟工具，支持长番茄钟、短番茄钟、长短休息与自定义计时，并提供手动修正、按日记录和周月年统计热力图。
...
- 可切换当前静止状态（长/短番茄钟、长/短休息、自定义），无需立即开始。
- 自定义计时器模式（不计入番茄钟统计）。
...
- 规则：长/短番茄钟 -> 默认休息，长/短休息 -> 默认番茄钟，自定义 -> 自定义。
- 非计时中：按“当前静止状态”启动对应模式。

### 右键菜单

- 会话：开始、暂停或继续、结束
- 切换：长番茄钟/短番茄钟/短休息/长休息/自定义计时（运行中也可直接切换）
- 待机：长番茄钟/短番茄钟/短休息/长休息/自定义计时（计时进行中不可切换）
